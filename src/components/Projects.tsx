...
const ProjectCard = memo(function ProjectCard({ 
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-[20px] bg-gradient-to-br from-primary/0 via-secondary/0 to-tertiary/0 group-hover:from-primary/10 group-hover:via-secondary/5 group-hover:to-tertiary/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
      
      {/* Card Header */}
      <div className="p-6 sm:p-8 pb-0 flex-grow relative z-10">
        <div className="flex justify-between items-start mb-6">
          <div className="p-4 rounded-2xl bg-gradient-to-br from-foreground/5 to-foreground/10 text-foreground group-hover:from-primary group-hover:to-secondary group-hover:text-white transition-all duration-300 shadow-lg">
      ...
      <div className="p-6 sm:p-8 pt-0 mt-auto relative z-10">
        ...
        <div className="flex flex-wrap gap-2">
          {project.technologies.map((tag) => (
            <span 
              key={tag} 
              className="px-4 py-1.5 text-xs font-mono font-semibold text-gray-600 dark:text-gray-300 bg-white/60 dark:bg-white/10 backdrop-blur-sm rounded-full border border-black/10 dark:border-white/20 hover:border-primary/50 hover:scale-105 transition-all duration-300"
            >
              {tag}
            </span>
          ))}
